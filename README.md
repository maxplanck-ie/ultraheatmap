...
If no annotation file is given, program checks for the exact match between regions name from the input matrix and finds the same name on the given feature tables. The following image presents a matrix has been generated by this module after mapping genes to peaks on a real data.
![addFeatureToMatrix example](./docs/content/images/ultraheatmap_ferrari.png)
